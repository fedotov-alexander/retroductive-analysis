...
def rerank_and_anonymize_players():
    ...
    It then selects the columns with this number, all the ids and the lifespan
    of the players (for ranking and segmentation on the id level).
    It then anonymises the players by removing their ids from the dataframe.
    It then writes the dataframes to parquet files.
    :return: None
    """
    config = go_configure()
    ...
    order = ['number', 'afid', 'country_code', 'client_id', 'first_seen', 'first_date', 'last_seen', 'last_date', 'lifespan', 'uid']
    combined = combined.select(order)
    print(combined.glimpse(return_as_string=True))
    try:
        assert_frame_equal(df, combined)  # This should raise an AssertionError
    except AssertionError as e:
        print(e)  # Output: DataFrames are not equal
